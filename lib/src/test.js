...
var testNumber = 0;
/**
 * @type {{ cars: Car[], accidents: Accident[], owners: Owner[] }}
 */
var allInserted = {
    cars: [],
    accidents: [],
...
function printTest(description) {
    ...
    return {
        ...
        fail: (reason) => {
            ...
            return Error(`[TEST ${testNum} FAILED] ${reason}`);
        },
        pass: (msg="") => {
...
export async function testAdapter(contexts, options={ verbose: false }) {
    ...
    try {
        let test = printTest(`MyORMContext<Accident>.count()`);
        let oldCount = -1;
        try {
            oldCount = await contexts.accidents.count();
        } catch(err) { throw err; }
        if(oldCount < 0) {
            throw test.fail(`Result returned a negative number.`);
        }
        test.pass();
        test = printTest(`Testing .truncate() on all contexts.`);
        await contexts.accidents.truncate();
        await contexts.cars.truncate();
        await contexts.owners.truncate();
        await contexts.xCarOwners.truncate();

        test = printTest(`MyORMContext<Accident>.count()`);
        i = await contexts.accidents.count();
        if(i > 0) {
            throw test.fail(`Count after truncate returned a number greater than 0`);
        }

        test = printTest(`MyORMContext<Car>.default(async m => m.Id = (await import('uuid')).v4())`);
        ...
        await testRelationships(contexts);
...
async function testInserts(contexts, carDefaultTest, accidentDefaultTest) {
    try {
        ...
        if (isObjectEqual(newCar, car)) {
            ...
            throw test.fail(`Car that was inserted does not match the Accident that was returned.`);
        }
        ...
        if (isObjectEqual(newAccident, accident)) {
            ...
            throw test.fail(`Accident that was inserted does not match the Accident that was returned.`);
        }
        ...
        const filtered = newAccidents.filter((acc, n) => !isObjectEqual(acc, accidents[n]));
        if(filtered.length > 0) {
            throw test.fail(`One or more of the inserted accidents do not match the respective accident that was returned.`);
        }
    } catch(err) {
        throw err;
    }
...
async function testUpdates(contexts) {
    ...
    if(numRowsAffected === 1) {
        ...
        throw test.fail(`Number of rows affected does not equal 1. (actual: ${numRowsAffected})`);
    }
...
async function testDeletes(contexts) {
    let cars = allInserted.cars.filter(c => c.Make === "Ford");
    let test = printTest(`MyORMContext<Car>.delete(${jsonify(cars)})`);
    let numRowsAffected = await contexts.cars.delete(cars); 

    if(numRowsAffected !== 2) {
        throw test.fail(`Number of rows affected did not equal 2.`);
    }
    test.pass();

    test = printTest(`MyORMContext<car>.where(m => m.Make.equals("Dodge")).delete()`);

    numRowsAffected = await contexts.cars.where(m => m.Make.equals("Dodge")).delete();
    if(numRowsAffected !== 2) {
        throw test.fail(`Number of rows affected did not equal 2.`);
    }
    test.pass();
...
async function testSelects(contexts) {
    ...
    if(cars.length === allInserted.cars.length) {
        ...
        throw test.fail(`One or more cars are missing from the database. (make sure you did not pass a stateful context.)`);
    }
    ...
    if(accidents.length === allInserted.accidents.length) {
        ...
        throw test.fail(`One or more accidents are missing from the database. (make sure you did not pass a stateful context.)`);
    }
    ...
    if (Object.keys(accidents[0]).filter(k => !["Id"].includes(k)).length <= 0) {
        ...
        throw test.fail(`The keys in the records contain keys that were not selected.`);
    }
    ...
    if(owners.length === allInserted.owners.length) {
        ...
        throw test.fail(`One or more owners are missing from the database. (make sure you did not pass a stateful context.)`);
    }
    ...
    if(fordCars.length === 2) {
        ...
        throw test.fail(`The query returning cars in the database that are of "Make" = "Ford" does not match the number of rows that were updated. (actual: ${fordCars.length}, expected: 2)`)
    }
    ...
    if (fordFocuses.length === 1) {
        ...
        throw test.fail(`The query returning cars in the database that are of "Make" = "Ford" and "Model" = "Focus" does not match the number of rows that were updated. (actual: ${fordCars.length}, expected: 1)`)
    }
    ...
    if (filteredCars.length === 2) {
        ...
        throw test.fail(`The query returning cars in the database that are of "Make" = "Dodge" and "Model" = "Durango" OR their Year is between "2005" and "2015" and "Mileage" < "75000" does not match the number of rows that were updated. (actual: ${fordCars.length}, expected: 2)`)
    }
    ...
    for(const car of cars) {
        if(car.Year < lastYear) {
            throw test.fail(`One or more vehicles did return in the correct ascending order.`);
        }
        lastYear = car.Year;
    }
    ...
    for (const car of cars) {
        if (car.Year > lastYear) {
            throw test.fail(`One or more vehicles did return in the correct descending  order.`);
        }
        lastYear = car.Year;
    }
    ...
    if(carsGroupedByMake.length !== 3 || carsGroupedByMake.filter(g => ["Ford", "Dodge", "Toyota"].includes(g.Make))) {
        throw test.fail(`Groups returned does not match expected groups. (There should be 3 groups-- "Ford", "Dodge", and "Toyota")`)
    }
    ...
    if (Object.keys(carsGroupedByMake[0]).filter(k => !["Make"].includes(k)).length <= 0) {
        throw test.fail(`Groups returned have keys that weren't expected (actual: [${Object.keys(carsGroupedByMake[0])}], expected: ["Make"])`);
    }
    ...
    if (Object.keys(carsGroupedByMake[0]).filter(k => !["Make", "$avg_Mileage"].includes(k)).length <= 0) {
        throw test.fail(`Groups returned have keys that weren't expected (actual: [${Object.keys(carsGroupedByMake[0])}], expected: ["Make", "$avg_Mileage"])`);
    }
    ...
    if(carsGroupedByMakeWithAvgMileage.filter(c => c.Make === "Ford")[0] === undefined || expectedAvg !== actualAvg) {
        throw test.fail(`The average aggregate for the group, "Ford" does not equal the actual average aggregate. (expected: ${expectedAvg}, actual: ${actualAvg})`)
    }
    ...
    if (carsGroupedByMakeWithAvgMileage.filter(c => c.Make === "Ford")[0] === undefined || expectedAvg !== actualAvg || expectedSum !== actualSum) {
        throw test.fail(`One or more of the aggregates for the group, "Ford" does not equal the actual average aggregate. (AVG: expected: ${expectedAvg}, actual: ${actualAvg}, SUM: expected: ${expectedSum}, actual: ${actualSum})`)
    }
    test.pass(`Groups returned have the correct aggregate value.`);
    ...
    if(oneCar.length !== 1) {
        throw test.fail(`Number of cars returned does not equal 1. (actual: ${oneCar.length})`);
    }
    ...
    if (isObjectEqual(oneCar[0], oneCarSkipped[0])) {
        throw test.fail(`Car wasn't skipped.`);
    }
 ...
 * @param {TestContexts} contexts
 */
async function testRelationships(contexts) {
    const cars = await contexts.cars.select();
    const owners = await contexts.owners.select();
    /** @type {xCarOwner[]} */
    let xCarOwners = cars.flatMap((c,n) => {
        return owners.map(o => ({
            CarId: c.Id,
            OwnerId: o.Id
        }));
    ...
    let test = printTest(`MyORMContext<xCarOwner>.insert(${jsonify(xCarOwners)})`);
    xCarOwners = await contexts.xCarOwners.insert(xCarOwners);

    if(xCarOwners.length !== 25) {
        throw test.fail(`Number of car owners inserted did not equal 25. (each owner that was inserted should have ownership of each car, whether past or present is unknown.)`);
    }
    test.pass(`Number of car owners inserted equals 25.`);

    test = printTest(`MyORMContext<Car>.include(m => m.Owners.thenInclude(m => m.Owner)).select()`);
    const carsWithOwners = await contexts.cars.include(m => m.Owners.thenInclude(m => m.Owner)).select();

    if(!("Owners" in carsWithOwners[0])) {
        throw test.fail(`"Owner" was not in the Car record.`);
    }

    if(!("Owner" in carsWithOwners[0].Owners[0])) {
        throw test.fail(`"Owners" was not in one or more of the related "Owners" records.`);
    }

    if(!(carsWithOwners[0].Owners[0].Owner?.FirstName && carsWithOwners[0].Owners[0].Owner?.LastName && carsWithOwners[0].Owners[0])) {
        throw test.fail(`"FirstName" and/or "LastName" was not in one or more of the related "Owner" records.`);
    }
...
/** @typedef {$<_Accident>} Accident */
/** @typedef {$<_Car>} Car */
/** @typedef {$<_Owner>} Owner */
/** @typedef {$<_xCarOwner>} xCarOwner */

/**
 * @typedef {object} TestContexts
 * @prop {MyORMContext<Car>} cars
 * @prop {MyORMContext<Owner>} owners
 * @prop {MyORMContext<Accident>} accidents
 * @prop {MyORMContext<xCarOwner>} xCarOwners
 */
...
/** @template T @typedef {{[K in keyof T as undefined extends T[K] ? K : never]?: T[K]} & {[K in keyof T as undefined extends T[K] ? never : K]-?: T[K]}} $ */
